...
function perder_nivel () {
    info.changeLifeBy(-1)
    game.showLongText("No has logrado superar el nivel", DialogLayout.Bottom)
    tiles.setCurrentTilemap(tilemap`nivel1`)
    tiles.placeOnTile(jugador, tiles.getTileLocation(6, 22))
    restablecer_gravedad()
}
function superar_nivel () {
    info.stopCountdown()
    info.changeScoreBy(1)
    game.showLongText("Nivel TUTORIAL superado!!", DialogLayout.Bottom)
